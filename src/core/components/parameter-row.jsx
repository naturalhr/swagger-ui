...
export default class ParameterRow extends Component {
  ...
  componentWillReceiveProps(props) {
    ...
    let parameterWithMeta =
      specSelectors.parameterWithMetaByIdentity(pathMethod, rawParam) ||
      new Map();
    // fallback, if the meta lookup fails
    parameterWithMeta = parameterWithMeta.isEmpty()
      ? rawParam
      : parameterWithMeta;
    ...
    let paramValue = parameterWithMeta
      ? parameterWithMeta.get("value")
      : undefined;
  ...
  onChangeIncludeEmpty = newValue => {
    ...
    return specActions.updateEmptyParamInclusion(
      pathMethod,
      paramName,
      paramIn,
      newValue
  ...
  setDefaultValue = () => {
    ...
    let paramWithMeta = specSelectors.parameterWithMetaByIdentity(
      pathMethod,
      rawParam
    );
    ...
    return (
      <div className="col-6 param_row">
        <td className="col parameters-col_name col_name">
          <div
            className={
              required ? "parameter__name required" : "parameter__name"
            }
          >
            {param.get("name")}
          ...
          <div className="parameter__deprecated">
            {isOAS3 && isOAS3() && param.get("deprecated")
              ? "deprecated"
              : null}
          </div>
          ...
          {!showCommonExtensions || !commonExt.size
            ? null
            : commonExt.map((v, key) => (
                <ParameterExt key={`${key}-${v}`} xKey={key} xVal={v} />
              ))}
          {!showExtensions || !extensions.size
            ? null
            : extensions.map((v, key) => (
                <ParameterExt key={`${key}-${v}`} xKey={key} xVal={v} />
              ))}
        </td>

        <td className="col parameters-col_description col_description">
          ...
          {!bodyParam && isExecute ? (
            <ParameterIncludeEmpty
              ...
              isIncluded={specSelectors.parameterInclusionSettingFor(
                pathMethod,
                param.get("name"),
                param.get("in")
              )}
